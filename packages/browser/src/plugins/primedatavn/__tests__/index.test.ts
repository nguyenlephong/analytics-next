...
import { primedatavn, PrimeDataVNSettings } from '..'
...
describe('Segment.io', () => {
  let options: PrimeDataVNSettings
  ...
  beforeEach(async () => {
    ...
    segment = primedatavn(analytics, options, {})
  ...
  describe('using a custom protocol', () => {
    it('should be able to send http requests', async () => {
      ...
      const segment = primedatavn(analytics, options, {})
      await analytics.register(segment, pageEnrichment)
      ...
      expect(url).toMatchInlineSnapshot(
        `"http://webapp-demo.primedatacdp.com/api/sdk/p"`
      )
    })
  })
  ...
  describe('configuring a keep alive', () => {
    ...
    it('should default to no keepalive', async () => {
      ...
      const segment = primedatavn(analytics, {
        apiKey: '',
      })
  ...
  describe('#page', () => {
    it('should enqueue section, name and properties', async () => {
      ...
      expect(url).toMatchInlineSnapshot(
        `"https://webapp-demo.primedatacdp.com/api/sdk/p"`
      )
  ...
  describe('#identify', () => {
    it('should enqueue an id and traits', async () => {
      ...
      const [url, params] = spyMock.mock.calls[0]
      expect(url).toMatchInlineSnapshot(
        `"https://webapp-demo.primedatacdp.com/api/sdk/i"`
      )
  ...
  describe('#track', () => {
    it('should enqueue an event and properties', async () => {
      ...
      const [url, params] = spyMock.mock.calls[0]
      expect(url).toMatchInlineSnapshot(
        `"https://webapp-demo.primedatacdp.com/api/sdk/t"`
      )
  ...
  describe('#group', () => {
    it('should enqueue groupId and traits', async () => {
      ...
      const [url, params] = spyMock.mock.calls[0]
      expect(url).toMatchInlineSnapshot(
        `"https://webapp-demo.primedatacdp.com/api/sdk/g"`
      )
  ...
  describe('#alias', () => {
    it('should enqueue .userId and .previousId', async () => {
      ...
      const [url, params] = spyMock.mock.calls[0]
      expect(url).toMatchInlineSnapshot(
        `"https://webapp-demo.primedatacdp.com/api/sdk/a"`
      )
